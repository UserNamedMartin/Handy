...
pub enum ShortcutActivation {
    ...
    /// (`hold_threshold_ms`).
    HoldOrToggle,
    /// Fork variant. Hold to record and release to stop, exactly like
    /// `HoldOrToggle` — but a *lone* tap does nothing at all, and it takes a
    /// **double** tap (two presses inside `double_tap_window_ms`) to lock
    /// recording on. Trades one deliberate gesture for immunity to a stray tap
    /// silently starting a recording, which matters when the transcribe key is
    /// a modifier like `fn`.
    #[default]
    HoldOrDoubleTap,
}
...
pub struct AppSettings {
    ...
    /// Hold-or-double-tap only: two presses closer together than this lock
    /// recording on. A longer gap reads as two lone taps, which do nothing.
...
pub fn get_default_settings() -> AppSettings {
    ...
    // Hands-free latch: while holding the transcribe key and recording,
...
fn apply_settings_migrations(
    ...
    if settings_value.get("shortcut_activation").is_none() {
        ...
        // Hybrid transcribe key meant hold-or-double-tap, not hold-only.
        // Reading the bool alone would silently drop hands-free for exactly the
        // users who had configured it.
        let fork_hybrid = settings_value
            .get("bindings")
            .and_then(|b| b.get("transcribe"))
            .and_then(|b| b.get("activation_mode"))
            .and_then(|m| m.as_str())
            .is_some_and(|m| m == "hybrid");
        if fork_hybrid {
            settings.shortcut_activation = ShortcutActivation::HoldOrDoubleTap;
            updated = true;
        } else if let Some(push_to_talk) =
            settings_value.get("push_to_talk").and_then(|v| v.as_bool())
        {
...
mod tests {
    ...
    fn empty_store_parses_with_defaults() {
        ...
        // Fork default: HoldOrDoubleTap, not upstream's HoldOrToggle — a lone
        // tap must never start a recording when the transcribe key is `fn`.
        assert_eq!(
            settings.shortcut_activation,
            ShortcutActivation::HoldOrDoubleTap
        );
    ...
    #[test]
    fn fork_hybrid_binding_migrates_to_hold_or_double_tap() {
        // A real pre-merge fork store: the global bool said push-to-talk, but
        // the transcribe binding overrode it with Hybrid. Reading the bool alone
        // would hand these users hold-only and silently remove hands-free.
        let mut settings = get_default_settings();
        settings.shortcut_activation = ShortcutActivation::Toggle;
        let raw = serde_json::json!({
            "push_to_talk": true,
            "bindings": {
                "transcribe": {
    ...
    #[test]
    fn non_hybrid_fork_store_still_honours_the_legacy_bool() {
        let mut settings = get_default_settings();
        let raw = serde_json::json!({
            "push_to_talk": true,
            "bindings": {
                "transcribe": { "id": "transcribe", "activation_mode": "global" }
            }
        });
    ...
    #[test]
    fn shortcut_activation_defaults_to_the_fork_mode_without_legacy_key() {
        let mut settings = get_default_settings();
