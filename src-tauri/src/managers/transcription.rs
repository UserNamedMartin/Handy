use crate::audio_toolkit::{
    ...
    remove_filler_words, whisper_autogain, OutputLanguageEvidence, SegmentClose, StreamSegmenter,
...
impl TranscriptionManager {
    ...
    fn run_whisper_segment_worker(
        ...
        if let Some(reply) = finalize_reply {
            // Whisper pseudo-streaming batch-transcribes each segment, so the
            // language question is settled the same way the offline path settles
            // it — downstream, from settings and the model's own report.
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        // `custom_words_sent_to_model` rather than upstream's `model_is_whisper`:
        // the question the corrector actually asks is "did the model already get
        // these words?", which is true for whisper's initial_prompt AND for
        // Gemini's custom_vocabulary.
        let filtered_result = post_process_transcription_text(
            result,
            &settings,
            custom_words_sent_to_model,
            &output_language,
            &model_languages,
        );
...
mod tests {
    ...
    fn stream_reply_keeps_real_text() {
        let reply = stream_reply(
            "Привет, deploy на staging.".to_string(),
            OutputLanguageEvidence::Unknown,
            Vec::new(),
        )
        .expect("real text must not fall back to batch");
        assert_eq!(reply.text, "Привет, deploy на staging.");
    }
