...
struct PendingRelease {
    ...
    /// `HoldOrDoubleTap` only: a short release opens this window rather than
    /// locking straight away.
    tap_window: Option<Duration>,
}

/// An unresolved tap in `HoldOrDoubleTap`: the key went down and came back up
/// inside the hold threshold, so it is either the first half of a double tap
/// (lock recording on) or a lone tap (do nothing). Recording continues until
/// the window closes, so the second press has something to latch onto.
struct PendingTap {
    binding_id: String,
    hotkey_string: String,
    deadline: Instant,
...
fn classify_busy_input(
    ...
    match (mode, is_pressed, remembered) {
        ...
        (PushToTalk | HoldOrToggle | HoldOrDoubleTap, true, None) => BusyAction::Remember,
        (PushToTalk | HoldOrToggle | HoldOrDoubleTap, true, Some(Remembered::Locked)) => BusyAction::Forget,
        (PushToTalk | HoldOrToggle | HoldOrDoubleTap, true, Some(Remembered::Held)) => BusyAction::Ignore,
        ...
        (PushToTalk | HoldOrToggle | HoldOrDoubleTap, false, _) => BusyAction::Ignore,
    }
}
...
struct InputEvent {
    ...
    external: bool,
    /// `HoldOrDoubleTap` only: how long a tap waits for its partner.
...
impl InputEvent {
    ...
    fn effective_hold_threshold(&self) -> Duration {
        match self.mode {
            ShortcutActivation::HoldOrToggle | ShortcutActivation::HoldOrDoubleTap => {
                self.hold_threshold
            }
    ...
    /// `Some(window)` only in `HoldOrDoubleTap`. Every other mode resolves a
    /// short release on the spot instead of waiting for a second tap.
    fn tap_window(&self) -> Option<Duration> {
        match self.mode {
            ShortcutActivation::HoldOrDoubleTap => Some(self.double_tap_window),
            _ => None,
        }
    }
}
...
enum Command {
    Input(InputEvent),
    /// The dedicated latch binding, registered only while recording.
    Latch,
...
impl CoordinatorState {
    ...
    /// Deadline of the deferred release, if any.
    fn grace_deadline(&self) -> Option<Instant> {
        self.pending_release.as_ref().map(|p| p.deadline)
    }

    /// Deadline of an unresolved double-tap, if any.
    fn tap_deadline(&self) -> Option<Instant> {
        self.pending_tap.as_ref().map(|p| p.deadline)
    }

    /// The next moment the coordinator must wake up on its own — drives
    /// `recv_timeout`. Both timers can be live at once (a release deferred by
    /// the grace window while a tap window is still open), so the loop sleeps
    /// until whichever lands first.
    fn next_deadline(&self) -> Option<Instant> {
        match (self.grace_deadline(), self.tap_deadline()) {
    ...
    /// The latch binding, pressed while a hold is live: keep recording after
    /// the transcribe key comes up. Same lock the double tap sets, reached by a
    /// different gesture.
    fn on_latch(&mut self) {
        self.pending_tap = None;
        if let Some(hold) = &mut self.hold {
            debug!("Latch pressed: recording locked on");
            hold.locked = true;
        }
    }
    ...
    fn on_input(&mut self, input: InputEvent, now: Instant) -> Option<Effect> {
        ...
        if input.is_pressed {
            match &self.stage {
                ...
                Stage::Recording(id) if id == &input.binding_id => {
                    // Second press inside the double-tap window: lock recording
                    // on. Checked before the locked/toggle handling below, which
                    // would otherwise read this press as "stop".
                    if self
                        .pending_tap
                        .as_ref()
                        .is_some_and(|t| t.binding_id == input.binding_id && now <= t.deadline)
                    {
                        self.pending_tap = None;
                        if let Some(hold) = &mut self.hold {
                            hold.locked = true;
                        }
                        debug!("Double tap for '{}': recording locked on", input.binding_id);
                        return None;
                    }
            ...
            && matches!(&self.stage, Stage::Recording(id) if id == &input.binding_id)
        {
            ...
            let tap_window = input.tap_window();
    ...
    fn finish_hold(
        ...
        if let Some(window) = tap_window {
            // HoldOrDoubleTap: one tap decides nothing. Keep recording and wait
            // for a partner; `on_tap_expired` stops it if none arrives.
            debug!("Tap ({held:?}) for '{binding_id}': waiting {window:?} for a second tap");
            self.pending_tap = Some(PendingTap {
                binding_id,
                hotkey_string,
                deadline: released_at + window,
            });
            return None;
        }
        if let Some(hold) = &mut self.hold {
...
impl TranscriptionCoordinator {
                    ...
                    let cmd = if let Some(deadline) = state.next_deadline() {
                        match rx.recv_timeout(deadline.saturating_duration_since(Instant::now())) {
                            ...
                            Err(mpsc::RecvTimeoutError::Timeout) => {
                                // Both timers can be live; resolve whichever is
                                // actually due, not whichever we checked first.
                                let now = Instant::now();
                                let effect = if state
                                    .grace_deadline()
                                    .is_some_and(|d| d <= now)
    ...
    /// The latch binding fired: lock the live recording on, so releasing the
    /// transcribe key no longer ends it. A no-op when nothing is recording.
    pub fn notify_latch(&self) {
        if self.tx.send(Command::Latch).is_err() {
            warn!("Coordinator channel closed; latch ignored");
        }
    }
...
mod tests {
    ...
    // its own test rather than sharing a fixture.

    #[test]
    fn hold_or_double_tap_long_hold_stops_on_release() {
    ...
    #[test]
    fn hold_or_double_tap_lone_tap_starts_nothing_lasting() {
        let mode = ShortcutActivation::HoldOrDoubleTap;
        let mut state = CoordinatorState::new();
        let t0 = Instant::now();
        ...
        // The tap opens a window instead of locking: upstream's mode would have
        // latched here, which is exactly what this variant exists to avoid.
    ...
    #[test]
    fn hold_or_double_tap_second_tap_locks_recording_on() {
        let mode = ShortcutActivation::HoldOrDoubleTap;
        let mut state = CoordinatorState::new();
        let t0 = Instant::now();
        ...
        // Second press inside the window: keep recording, hands free.
        assert!(state.on_input(input(mode, true), t0 + ms(250)).is_none());
        assert!(state.is_locked());
        assert_eq!(state.stage, Stage::Recording(BINDING.to_string()));
        assert!(state.tap_deadline().is_none());
        // Its release must not end the locked session.
        assert!(state.on_input(input(mode, false), t0 + ms(300)).is_none());
        assert_eq!(state.stage, Stage::Recording(BINDING.to_string()));

        // A later press finishes it.
        assert!(matches!(
            state.on_input(input(mode, true), t0 + ms(9000)),
            Some(Effect::Stop { .. })
        ));
    }

    #[test]
    fn hold_or_double_tap_second_tap_after_the_window_does_not_lock() {
        let mode = ShortcutActivation::HoldOrDoubleTap;
        let mut state = CoordinatorState::new();
        let t0 = Instant::now();
    ...
    #[test]
    fn latch_locks_a_live_hold_so_the_key_can_be_released() {
        let mode = ShortcutActivation::HoldOrDoubleTap;
        let mut state = CoordinatorState::new();
        let t0 = Instant::now();
        ...
        // The dedicated latch binding, pressed mid-hold.
        state.on_latch();
        assert!(state.is_locked());

        // Releasing the transcribe key no longer ends the session.
        assert!(state.on_input(input(mode, false), t0 + ms(2000)).is_none());
        assert_eq!(state.stage, Stage::Recording(BINDING.to_string()));
        assert!(matches!(
            state.on_input(input(mode, true), t0 + ms(6000)),
            Some(Effect::Stop { .. })
        ));
    }

    #[test]
    fn cancel_clears_a_pending_double_tap_window() {
        let mode = ShortcutActivation::HoldOrDoubleTap;
        let mut state = CoordinatorState::new();
        let t0 = Instant::now();
        ...
        assert!(
            state.tap_deadline().is_none(),
            "Escape must not leave a window that later stops a recording nobody started"
        );
    }

    #[test]
    fn next_deadline_is_the_nearer_of_the_two_timers() {
        let mut state = CoordinatorState::new();
        ...
        state.on_input(input(mode, true), t0);
        state.on_input(input(mode, false), t0 + ms(120));
        // Release deferred by the grace window; no tap window yet.
        assert_eq!(state.next_deadline(), state.grace_deadline());

        state.on_grace_expired();
        // Now only the tap window is live.
        assert_eq!(state.next_deadline(), state.tap_deadline());
    }
