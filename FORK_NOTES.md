...
**Overlay tuner (design tool, outside this repo):** the overlay look was dialled in with a standalone HTML tuner at `~/tools-for-agents/handy-tuner/index.html` — a live, real-mic preview with sliders whose "config" readout maps 1:1 to the CSS values above. Serve it for editing with `python3 -m http.server 8787 --bind 127.0.0.1 --directory ~/tools-for-agents/handy-tuner` and open <http://localhost:8787/>. Not part of the app build.
